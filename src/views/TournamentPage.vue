...
const secondPart = urlPath.split('/');
let namePath = ref(null);
...
const allTitleTournament = computed(() => namePath === 'tournamentsAll');
const allTournamentArr = [];

if (secondPart.length > 2) {
  namePath = secondPart[2];
}
if (allTitleTournament.value) {
  title.value = 'All';
} else {
  title.value = tournamentsNameGame[0][namePath].title;
  webp.value = tournamentsNameGame[0][namePath].webp;
  png.value = tournamentsNameGame[0][namePath].png;
}
      ...
      <div class="container">
        <div class="tourney-game__content">
          <div class="tourney-game__row">
            ...
            <div class="tourney-game__img" v-if="!allTitleTournament">
          ...
          <div class="tourney-game__column">
            ...
            <div :class="['tourney-game__filter', { 'tourney-game__filter_act': isOpenFilter }]">
              <div class="tourney-game__filter-row" @click="toggleFilter">
                ...
                <button class="tourney-game__filter-clear" v-on:click="toggleFilter">clear</button>
              </div>
              ...
              >
                <UIFilter
                  :arr="group"
                  :label="group[0].title"
                  @click="filterTournaments(group[0].class, $event)"
                />
              </div>
            </div>
          </div>
